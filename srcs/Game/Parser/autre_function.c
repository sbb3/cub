...
/*   Updated: 2022/10/13 10:22:37 by labenall         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (cheak_name_texure(map_struct->ea->content) == 1)
		exit_erreur("texure eo must finish by .xpm");
	if (cheak_name_texure(map_struct->we->content) == 1)
		exit_erreur("texure we must finish by .xpm");
	if (cheak_name_texure(map_struct->no->content) == 1)
		exit_erreur("texure no must finish by .xpm");
	if (cheak_name_texure(map_struct->so->content) == 1)
